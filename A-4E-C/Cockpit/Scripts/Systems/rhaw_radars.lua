-- Dumped using export script
units = {
  ["1L13 EWR"] = {
    DisplayName = "EWR 1L13",
    frequencies = { { 180000000, 220000000 } },
    radars = { "1L13 EWR" },
    type = "1L13 EWR"
  },
  ["2S6 Tunguska"] = {
    DisplayName = 'SAM SA-19 Tunguska "Grison" ',
    frequencies = { { 2000000000, 3000000000 } },
    radars = { "2S6 Tunguska" },
    type = "2S6 Tunguska"
  },
  ["55G6 EWR"] = {
    DisplayName = "EWR 55G6",
    frequencies = { { 30000000, 300000000 } },
    radars = { "55G6 EWR" },
    type = "55G6 EWR"
  },
  ["A-50"] = {
    DisplayName = "A-50",
    frequencies = {},
    radars = { "Shmel" },
    type = "A-50"
  },
  ["AH-64D"] = {
    DisplayName = "AH-64D",
    frequencies = {},
    radars = { "AN/APG-78" },
    type = "AH-64D"
  },
  ["AH-64D_BLK_II"] = {
    DisplayName = "AH-64D BLK.II",
    frequencies = {},
    radars = { "AN/APG-78" },
    type = "AH-64D_BLK_II"
  },
  AJS37 = {
    DisplayName = "AJS37",
    frequencies = {},
    radars = { "PS-37A" },
    type = "AJS37"
  },
  ALBATROS = {
    DisplayName = "Corvette 1124.4 Grisha",
    frequencies = { { 6000000000, 8000000000 } },
    radars = { "Osa 9A33 ln", "albatros search radar" },
    type = "ALBATROS"
  },
  ["B-1B"] = {
    DisplayName = "B-1B",
    frequencies = {},
    radars = { "B-1B SS radar" },
    type = "B-1B"
  },
  ["B-52H"] = {
    DisplayName = "B-52H",
    frequencies = {},
    radars = { "B-52H SS radar" },
    type = "B-52H"
  },
  ["BDK-775"] = {
    DisplayName = "LS Ropucha",
    frequencies = {},
    radars = { "rezki search radar" },
    type = "BDK-775"
  },
  ["C-101CC"] = {
    DisplayName = "C-101CC",
    frequencies = {},
    radars = { "Tornado SS radar" },
    type = "C-101CC"
  },
  CVN_71 = {
    DisplayName = "CVN-71 Theodore Roosevelt",
    frequencies = { { 2000000000, 4000000000 }, { 850000000, 942000000 } },
    radars = { "seasparrow tr", "carrier search radar" },
    type = "CVN_71"
  },
  CVN_72 = {
    DisplayName = "CVN-72 Abraham Lincoln",
    frequencies = { { 2000000000, 4000000000 }, { 850000000, 942000000 } },
    radars = { "seasparrow tr", "carrier search radar" },
    type = "CVN_72"
  },
  CVN_73 = {
    DisplayName = "CVN-73 George Washington",
    frequencies = { { 2000000000, 4000000000 }, { 850000000, 942000000 } },
    radars = { "seasparrow tr", "carrier search radar" },
    type = "CVN_73"
  },
  CVN_75 = {
    DisplayName = "CVN-75 Harry S. Truman",
    frequencies = { { 2000000000, 4000000000 }, { 850000000, 942000000 } },
    radars = { "seasparrow tr", "carrier search radar" },
    type = "CVN_75"
  },
  CV_1143_5 = {
    DisplayName = "CV 1143.5 Admiral Kuznetsov(2017)",
    frequencies = { { 2000000000, 3000000000 }, { 8000000000, 12000000000 } },
    radars = { "Tor 9A331", "2S6 Tunguska", "carrier search radar" },
    type = "CV_1143_5"
  },
  ["Dog Ear radar"] = {
    DisplayName = 'MCC-SR Sborka "Dog Ear" SR',
    frequencies = { { 3000000000, 6000000000 } },
    radars = { "Dog Ear radar" },
    type = "Dog Ear radar"
  },
  ["E-2C"] = {
    DisplayName = "E-2D",
    frequencies = {},
    radars = { "AN/APS-138" },
    type = "E-2C"
  },
  ["E-3A"] = {
    DisplayName = "E-3A",
    frequencies = {},
    radars = { "AN/APY-1" },
    type = "E-3A"
  },
  ["F-14A"] = {
    DisplayName = "F-14A",
    frequencies = {},
    radars = { "AN/APG-71" },
    type = "F-14A"
  },
  ["F-14A-135-GR"] = {
    DisplayName = "F-14A-135-GR",
    frequencies = {},
    radars = { "AN/APG-71" },
    type = "F-14A-135-GR"
  },
  ["F-14B"] = {
    DisplayName = "F-14B",
    frequencies = {},
    radars = { "AN/APG-71" },
    type = "F-14B"
  },
  ["F-15C"] = {
    DisplayName = "F-15C",
    frequencies = {},
    radars = { "AN/APG-63" },
    type = "F-15C"
  },
  ["F-15E"] = {
    DisplayName = "F-15E",
    frequencies = {},
    radars = { "AN/APG-63" },
    type = "F-15E"
  },
  ["F-15ESE"] = {
    DisplayName = "F-15E S4+",
    frequencies = {},
    radars = { "AN/APG-63" },
    type = "F-15ESE"
  },
  ["F-16A"] = {
    DisplayName = "F-16A",
    frequencies = {},
    radars = { "AN/APG-68" },
    type = "F-16A"
  },
  ["F-16A MLU"] = {
    DisplayName = "F-16A MLU",
    frequencies = {},
    radars = { "AN/APG-68" },
    type = "F-16A MLU"
  },
  ["F-16C bl.50"] = {
    DisplayName = "F-16C bl.50",
    frequencies = {},
    radars = { "AN/APG-68" },
    type = "F-16C bl.50"
  },
  ["F-16C bl.52d"] = {
    DisplayName = "F-16C bl.52d",
    frequencies = {},
    radars = { "AN/APG-68" },
    type = "F-16C bl.52d"
  },
  ["F-16C_50"] = {
    DisplayName = "F-16CM bl.50",
    frequencies = {},
    radars = { "AN/APG-68" },
    type = "F-16C_50"
  },
  ["F-4E"] = {
    DisplayName = "F-4E",
    frequencies = {},
    radars = { "AN/APQ-120" },
    type = "F-4E"
  },
  ["F-4E-45MC"] = {
    DisplayName = "F-4E-45MC",
    frequencies = {},
    radars = { "HB_ANAPQ_120" },
    type = "F-4E-45MC"
  },
  ["F-5E"] = {
    DisplayName = "F-5E",
    frequencies = {},
    radars = { "AN/APQ-153" },
    type = "F-5E"
  },
  ["F-5E-3"] = {
    DisplayName = "F-5E-3",
    frequencies = {},
    radars = { "AN/APQ-159" },
    type = "F-5E-3"
  },
  ["F-5E-3_FC"] = {
    DisplayName = "F-5E FC",
    frequencies = {},
    radars = { "AN/APQ-159" },
    type = "F-5E-3_FC"
  },
  ["F/A-18A"] = {
    DisplayName = "F/A-18A",
    frequencies = {},
    radars = { "AN/APG-73" },
    type = "F/A-18A"
  },
  ["F/A-18C"] = {
    DisplayName = "F/A-18C",
    frequencies = {},
    radars = { "AN/APG-73" },
    type = "F/A-18C"
  },
  ["FA-18C_hornet"] = {
    DisplayName = "F/A-18C Lot 20",
    frequencies = {},
    radars = { "AN/APG-73" },
    type = "FA-18C_hornet"
  },
  ["FPS-117"] = {
    DisplayName = "EWR AN/FPS-117 Radar",
    frequencies = { { 1215000000, 1400000000 } },
    radars = { "FPS-117" },
    type = "FPS-117"
  },
  ["FPS-117 Dome"] = {
    DisplayName = "EWR AN/FPS-117 Radar (domed)",
    frequencies = { { 1215000000, 1400000000 } },
    radars = { "FPS-117" },
    type = "FPS-117 Dome"
  },
  Forrestal = {
    DisplayName = "CV-59 Forrestal",
    frequencies = {},
    radars = { "seasparrow tr", "carrier search radar" },
    type = "Forrestal"
  },
  Gepard = {
    DisplayName = "SPAAA Gepard",
    frequencies = { { 2000000000, 3000000000 } },
    radars = { "Gepard" },
    type = "Gepard"
  },
  ["H-6J"] = {
    DisplayName = "H-6J",
    frequencies = {},
    radars = { "H-6J RADAR" },
    type = "H-6J"
  },
  ["HEMTT_C-RAM_Phalanx"] = {
    DisplayName = "LPWS C-RAM",
    frequencies = { { 12000000000, 18000000000 } },
    radars = { "C_RAM_Phalanx" },
    type = "HEMTT_C-RAM_Phalanx"
  },
  ["HQ-7_LN_P"] = {
    DisplayName = "HQ-7 SHORAD TELAR (Player)",
    frequencies = { { 50000000, 54000000 }, { 2000000000, 2200000000 } },
    radars = { "HQ-7 TR" },
    type = "HQ-7_LN_P"
  },
  ["HQ-7_LN_SP"] = {
    DisplayName = "HQ-7B SHORAD TELAR",
    frequencies = { { 50000000, 54000000 }, { 2000000000, 2200000000 } },
    radars = { "HQ-7 TR" },
    type = "HQ-7_LN_SP"
  },
  ["HQ-7_STR_SP"] = {
    DisplayName = "HQ-7B SHORAD SR",
    frequencies = { { 50000000, 54000000 }, { 2000000000, 2200000000 } },
    radars = { "HQ-7 SR" },
    type = "HQ-7_STR_SP"
  },
  ["Hawk cwar"] = {
    DisplayName = "SAM Hawk CWAR AN/MPQ-55",
    frequencies = { { 10000000000, 20000000000 } },
    radars = { "Hawk sr" },
    type = "Hawk cwar"
  },
  ["Hawk sr"] = {
    DisplayName = "SAM Hawk SR (AN/MPQ-50)",
    frequencies = { { 500000000, 1000000000 } },
    radars = { "Hawk sr" },
    type = "Hawk sr"
  },
  ["Hawk tr"] = {
    DisplayName = "SAM Hawk TR (AN/MPQ-46)",
    frequencies = {},
    radars = { "Hawk tr" },
    type = "Hawk tr"
  },
  ["J-11A"] = {
    DisplayName = "J-11A",
    frequencies = {},
    radars = { "N-001" },
    type = "J-11A"
  },
  ["JF-17"] = {
    DisplayName = "JF-17",
    frequencies = {},
    radars = { "KLJ-7" },
    type = "JF-17"
  },
  ["KJ-2000"] = {
    DisplayName = "KJ-2000",
    frequencies = {},
    radars = { "AESA_KJ2000" },
    type = "KJ-2000"
  },
  KUZNECOW = {
    DisplayName = "CV 1143.5 Admiral Kuznetsov",
    frequencies = { { 2000000000, 3000000000 }, { 8000000000, 12000000000 } },
    radars = { "Tor 9A331", "2S6 Tunguska", "carrier search radar" },
    type = "KUZNECOW"
  },
  ["Kub 1S91 str"] = {
    DisplayName = 'SAM SA-6 Kub "Straight Flush" STR',
    frequencies = { { 500000000, 1000000000 } },
    radars = { "Kub 1S91 str" },
    type = "Kub 1S91 str"
  },
  LHA_Tarawa = {
    DisplayName = "LHA-1 Tarawa",
    frequencies = {},
    radars = { "seasparrow tr", "carrier search radar" },
    type = "LHA_Tarawa"
  },
  La_Combattante_II = {
    DisplayName = "FAC La Combattante IIa",
    frequencies = {},
    radars = { "molniya search radar" },
    type = "La_Combattante_II"
  },
  ["M-2000C"] = {
    DisplayName = "M-2000C",
    frequencies = {},
    radars = { "RDY" },
    type = "M-2000C"
  },
  MOLNIYA = {
    DisplayName = "Corvette 1241.1 Molniya",
    frequencies = { { 2000000000, 3000000000 }, { 8000000000, 12000000000 } },
    radars = { "molniya search radar" },
    type = "MOLNIYA"
  },
  MOSCOW = {
    DisplayName = "Cruiser 1164 Moskva",
    frequencies = { { 2000000000, 2500000000 }, { 8000000000, 12000000000 } },
    radars = { "S-300PS 40B6M tr navy", "Osa 9A33 ln", "moskva search radar" },
    type = "MOSCOW"
  },
  ["MQ-9 Reaper"] = {
    DisplayName = "MQ-9 Reaper",
    frequencies = {},
    radars = { "RQ-1 Predator SAR" },
    type = "MQ-9 Reaper"
  },
  ["MiG-19P"] = {
    DisplayName = "MiG-19P",
    frequencies = {},
    radars = { "N-008" },
    type = "MiG-19P"
  },
  ["MiG-21Bis"] = {
    DisplayName = "MiG-21Bis",
    frequencies = {},
    radars = { "N-008" },
    type = "MiG-21Bis"
  },
  ["MiG-23MLD"] = {
    DisplayName = "MiG-23MLD",
    frequencies = {},
    radars = { "N-008" },
    type = "MiG-23MLD"
  },
  ["MiG-25PD"] = {
    DisplayName = "MiG-25PD",
    frequencies = {},
    radars = { "N-005" },
    type = "MiG-25PD"
  },
  ["MiG-25RBT"] = {
    DisplayName = "MiG-25RBT",
    frequencies = {},
    radars = { "Orion-A" },
    type = "MiG-25RBT"
  },
  ["MiG-29A"] = {
    DisplayName = "MiG-29A",
    frequencies = {},
    radars = { "N-019" },
    type = "MiG-29A"
  },
  ["MiG-29G"] = {
    DisplayName = "MiG-29G",
    frequencies = {},
    radars = { "N-019" },
    type = "MiG-29G"
  },
  ["MiG-29S"] = {
    DisplayName = "MiG-29S",
    frequencies = {},
    radars = { "N-019M" },
    type = "MiG-29S"
  },
  ["MiG-31"] = {
    DisplayName = "MiG-31",
    frequencies = {},
    radars = { "BRLS-8B" },
    type = "MiG-31"
  },
  ["Mirage 2000-5"] = {
    DisplayName = "Mirage 2000-5",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage 2000-5"
  },
  ["Mirage-F1AD"] = {
    DisplayName = "Mirage F1AD",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1AD"
  },
  ["Mirage-F1AZ"] = {
    DisplayName = "Mirage F1AZ",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1AZ"
  },
  ["Mirage-F1B"] = {
    DisplayName = "Mirage F1B",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1B"
  },
  ["Mirage-F1BD"] = {
    DisplayName = "Mirage F1BD",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1BD"
  },
  ["Mirage-F1BE"] = {
    DisplayName = "Mirage F1BE",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1BE"
  },
  ["Mirage-F1BQ"] = {
    DisplayName = "Mirage F1BQ",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1BQ"
  },
  ["Mirage-F1C"] = {
    DisplayName = "Mirage F1C",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1C"
  },
  ["Mirage-F1C-200"] = {
    DisplayName = "Mirage F1C-200",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1C-200"
  },
  ["Mirage-F1CE"] = {
    DisplayName = "Mirage F1CE",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1CE"
  },
  ["Mirage-F1CG"] = {
    DisplayName = "Mirage F1CG",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1CG"
  },
  ["Mirage-F1CH"] = {
    DisplayName = "Mirage F1CH",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1CH"
  },
  ["Mirage-F1CJ"] = {
    DisplayName = "Mirage F1CJ",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1CJ"
  },
  ["Mirage-F1CK"] = {
    DisplayName = "Mirage F1CK",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1CK"
  },
  ["Mirage-F1CR"] = {
    DisplayName = "Mirage F1CR",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1CR"
  },
  ["Mirage-F1CT"] = {
    DisplayName = "Mirage F1CT",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1CT"
  },
  ["Mirage-F1CZ"] = {
    DisplayName = "Mirage F1CZ",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1CZ"
  },
  ["Mirage-F1DDA"] = {
    DisplayName = "Mirage F1DDA",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1DDA"
  },
  ["Mirage-F1ED"] = {
    DisplayName = "Mirage F1ED",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1ED"
  },
  ["Mirage-F1EDA"] = {
    DisplayName = "Mirage F1EDA",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1EDA"
  },
  ["Mirage-F1EE"] = {
    DisplayName = "Mirage F1EE",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1EE"
  },
  ["Mirage-F1EH"] = {
    DisplayName = "Mirage F1EH",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1EH"
  },
  ["Mirage-F1EQ"] = {
    DisplayName = "Mirage F1EQ",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1EQ"
  },
  ["Mirage-F1JA"] = {
    DisplayName = "Mirage F1JA",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1JA"
  },
  ["Mirage-F1M-CE"] = {
    DisplayName = "Mirage F1M (C.14 1-25/32-51)",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1M-CE"
  },
  ["Mirage-F1M-EE"] = {
    DisplayName = "Mirage F1M (C.14 52-73)",
    frequencies = {},
    radars = { "RDY" },
    type = "Mirage-F1M-EE"
  },
  NASAMS_Radar_MPQ64F1 = {
    DisplayName = "SAM NASAMS SR MPQ64F1",
    frequencies = { { 8000000000, 12000000000 } },
    radars = { "NASAMS_Radar_MPQ64F1" },
    type = "NASAMS_Radar_MPQ64F1"
  },
  NEUSTRASH = {
    DisplayName = "Frigate 11540 Neustrashimy",
    frequencies = { { 2000000000, 3000000000 }, { 8000000000, 12000000000 } },
    radars = { "Tor 9A331", "2S6 Tunguska", "neustrashimy search radar" },
    type = "NEUSTRASH"
  },
  ["Osa 9A33 ln"] = {
    DisplayName = 'SAM SA-8 Osa "Gecko" TEL',
    frequencies = { { 500000000, 1000000000 } },
    radars = { "Osa 9A33 ln" },
    type = "Osa 9A33 ln"
  },
  PERRY = {
    DisplayName = "FFG Oliver Hazard Perry",
    frequencies = { { 850000000, 942000000 }, { 10250000000, 10500000000 } },
    radars = { "Patriot str", "perry search radar" },
    type = "PERRY"
  },
  PIOTR = {
    DisplayName = "Battlecruiser 1144.2 Pyotr Velikiy",
    frequencies = { { 2000000000, 3000000000 }, { 8000000000, 12000000000 } },
    radars = { "S-300PS 40B6M tr navy", "Tor 9A331", "2S6 Tunguska", "piotr velikiy search radar" },
    type = "PIOTR"
  },
  ["Patriot ECS"] = {
    DisplayName = "SAM Patriot ECS",
    frequencies = {},
    radars = { "Patriot str" },
    type = "Patriot ECS"
  },
  ["Patriot str"] = {
    DisplayName = "SAM Patriot STR",
    frequencies = { { 4480000000, 5450000000 } },
    radars = { "Patriot str" },
    type = "Patriot str"
  },
  RD_75 = {
    DisplayName = "SAM SA-2 S-75 RD-75 Amazonka RF",
    frequencies = {},
    radars = { "snr s-125 tr" },
    type = "RD_75"
  },
  REZKY = {
    DisplayName = "Frigate 1135M Rezky",
    frequencies = { { 1000000000, 1200000000 }, { 6000000000, 8000000000 } },
    radars = { "Osa 9A33 ln", "rezki search radar" },
    type = "REZKY"
  },
  RLS_19J6 = {
    DisplayName = 'SAM SA-5 S-200 ST-68U "Tin Shield" SR',
    frequencies = { { 2850000000, 3200000000 } },
    radars = { "RLS_19J6" },
    type = "RLS_19J6"
  },
  RPC_5N62V = {
    DisplayName = 'SAM SA-5 S-200 "Square Pair" TR',
    frequencies = { { 1550000000, 3900000000 } },
    radars = { "RPC S-200 TR" },
    type = "RPC_5N62V"
  },
  ["Roland ADS"] = {
    DisplayName = "SAM Roland ADS",
    frequencies = { { 1000000000, 2000000000 } },
    radars = { "Roland ADS" },
    type = "Roland ADS"
  },
  ["Roland Radar"] = {
    DisplayName = "SAM Roland EWR",
    frequencies = { { 1000000000, 2000000000 } },
    radars = { "Roland Radar" },
    type = "Roland Radar"
  },
  ["S-300PS 40B6M tr"] = {
    DisplayName = 'SAM SA-10 S-300 "Grumble" Flap Lid-A TR',
    frequencies = { { 8000000000, 20000000000 } },
    radars = { "S-300PS 40B6M tr" },
    type = "S-300PS 40B6M tr"
  },
  ["S-300PS 40B6MD sr"] = {
    DisplayName = 'SAM SA-10 S-300 "Grumble" Clam Shell SR',
    frequencies = { { 2900000000, 3300000000 } },
    radars = { "S-300PS 40B6MD sr" },
    type = "S-300PS 40B6MD sr"
  },
  ["S-300PS 40B6MD sr_19J6"] = {
    DisplayName = 'SAM SA-10 S-300 "Grumble" Tin Shield SR',
    frequencies = { { 2850000000, 3200000000 } },
    radars = { "RLS_19J6" },
    type = "S-300PS 40B6MD sr_19J6"
  },
  ["S-300PS 5H63C 30H6_tr"] = {
    DisplayName = 'SAM SA-10 S-300 "Grumble" Flap Lid-B TR',
    frequencies = { { 8000000000, 20000000000 } },
    radars = { "S-300PS 40B6M tr" },
    type = "S-300PS 5H63C 30H6_tr"
  },
  ["S-300PS 64H6E sr"] = {
    DisplayName = 'SAM SA-10 S-300 "Grumble" Big Bird SR',
    frequencies = { { 2900000000, 3300000000 } },
    radars = { "S-300PS 40B6MD sr" },
    type = "S-300PS 64H6E sr"
  },
  ["S-3B"] = {
    DisplayName = "S-3B",
    frequencies = {},
    radars = { "AN/APS-137" },
    type = "S-3B"
  },
  ["S-3B Tanker"] = {
    DisplayName = "S-3B Tanker",
    frequencies = {},
    radars = { "AN/APS-137" },
    type = "S-3B Tanker"
  },
  ["SA-11 Buk LN 9A310M1"] = {
    DisplayName = 'SAM SA-11 Buk "Gadfly" Fire Dome TEL',
    frequencies = { { 6000000000, 10000000000 } },
    radars = { "SA-11 Buk TR" },
    type = "SA-11 Buk LN 9A310M1"
  },
  ["SA-11 Buk SR 9S18M1"] = {
    DisplayName = 'SAM SA-11 Buk "Gadfly" Snow Drift SR',
    frequencies = { { 3000000000, 4000000000 } },
    radars = { "SA-11 Buk SR 9S18M1" },
    type = "SA-11 Buk SR 9S18M1"
  },
  ["SH-60B"] = {
    DisplayName = "SH-60B",
    frequencies = {},
    radars = { "AN/APS-142" },
    type = "SH-60B"
  },
  SNR_75V = {
    DisplayName = 'SAM SA-2 S-75 "Fan Song" TR',
    frequencies = { { 4910000000, 4990000000 }, { 5010000000, 5090000000 } },
    radars = { "snr s-125 tr" },
    type = "SNR_75V"
  },
  SON_9 = {
    DisplayName = "AAA Fire Can SON-9",
    frequencies = { { 2700000000, 2900000000 } },
    radars = { "son-9 tr" },
    type = "SON_9"
  },
  Silkworm_SR = {
    DisplayName = "AShM Silkworm SR",
    frequencies = {},
    radars = { "Mys-M1_SR" },
    type = "Silkworm_SR"
  },
  Stennis = {
    DisplayName = "CVN-74 John C. Stennis",
    frequencies = { { 2000000000, 4000000000 }, { 850000000, 942000000 } },
    radars = { "seasparrow tr", "carrier search radar" },
    type = "Stennis"
  },
  ["Su-24M"] = {
    DisplayName = "Su-24M",
    frequencies = {},
    radars = { "Orion-A" },
    type = "Su-24M"
  },
  ["Su-24MR"] = {
    DisplayName = "Su-24MR",
    frequencies = {},
    radars = { "Orion-A" },
    type = "Su-24MR"
  },
  ["Su-27"] = {
    DisplayName = "Su-27",
    frequencies = {},
    radars = { "N-001" },
    type = "Su-27"
  },
  ["Su-30"] = {
    DisplayName = "Su-30",
    frequencies = {},
    radars = { "N-011M" },
    type = "Su-30"
  },
  ["Su-33"] = {
    DisplayName = "Su-33",
    frequencies = {},
    radars = { "N-001" },
    type = "Su-33"
  },
  ["Su-34"] = {
    DisplayName = "Su-34",
    frequencies = {},
    radars = { "N-011M" },
    type = "Su-34"
  },
  TICONDEROG = {
    DisplayName = "CG Ticonderoga",
    frequencies = { { 3100000, 3500000 }, { 850000000, 942000000 } },
    radars = { "AEGIS_search_radar", "ticonderoga search radar" },
    type = "TICONDEROG"
  },
  ["Tor 9A331"] = {
    DisplayName = 'SAM SA-15 Tor "Gauntlet"',
    frequencies = { { 3000000000, 4000000000 } },
    radars = { "Tor 9A331" },
    type = "Tor 9A331"
  },
  ["Tornado GR4"] = {
    DisplayName = "Tornado GR4",
    frequencies = {},
    radars = { "Tornado SS radar" },
    type = "Tornado GR4"
  },
  ["Tornado IDS"] = {
    DisplayName = "Tornado IDS",
    frequencies = {},
    radars = { "Tornado SS radar" },
    type = "Tornado IDS"
  },
  ["Tu-142"] = {
    DisplayName = "Tu-142",
    frequencies = {},
    radars = { "Berkut-95" },
    type = "Tu-142"
  },
  ["Tu-160"] = {
    DisplayName = "Tu-160",
    frequencies = {},
    radars = { "Poisk" },
    type = "Tu-160"
  },
  ["Tu-22M3"] = {
    DisplayName = "Tu-22M3",
    frequencies = {},
    radars = { "PNA-D Leninets" },
    type = "Tu-22M3"
  },
  ["Tu-95MS"] = {
    DisplayName = "Tu-95MS",
    frequencies = {},
    radars = { "Rubidy MM" },
    type = "Tu-95MS"
  },
  Type_052B = {
    DisplayName = "Type 052B Destroyer",
    frequencies = { { 2000000000, 2500000000 }, { 8000000000, 12000000000 } },
    radars = { "PLAN Search Radar B", "052B SAM SR", "052B SAM TR" },
    type = "Type_052B"
  },
  Type_052C = {
    DisplayName = "Type 052C Destroyer",
    frequencies = { { 2000000000, 3000000000 }, { 8000000000, 12000000000 } },
    radars = { "PLAN Search Radar A", "052C SAM STR" },
    type = "Type_052C"
  },
  Type_054A = {
    DisplayName = "Type 054A Frigate",
    frequencies = { { 2000000000, 2500000000 }, { 8000000000, 12000000000 } },
    radars = { "PLAN Search Radar B", "052B SAM SR", "052B SAM TR" },
    type = "Type_054A"
  },
  Type_071 = {
    DisplayName = "Type 071 Amphibious Transport Dock",
    frequencies = {},
    radars = { "PLAN Search Radar B" },
    type = "Type_071"
  },
  Type_093 = {
    DisplayName = "Type 093 Attack Submarine",
    frequencies = {},
    radars = { "PLAN Search Radar B", "052B SAM TR" },
    type = "Type_093"
  },
  USS_Arleigh_Burke_IIa = {
    DisplayName = "DDG Arleigh Burke IIa",
    frequencies = { { 3100000, 3500000 }, { 850000000, 942000000 } },
    radars = { "AEGIS_search_radar", "ticonderoga search radar" },
    type = "USS_Arleigh_Burke_IIa"
  },
  VINSON = {
    DisplayName = "CVN-70 Carl Vinson",
    frequencies = { { 2000000000, 4000000000 }, { 850000000, 942000000 } },
    radars = { "seasparrow tr", "carrier search radar" },
    type = "VINSON"
  },
  ["ZSU-23-4 Shilka"] = {
    DisplayName = 'SPAAA ZSU-23-4 Shilka "Gun Dish"',
    frequencies = {},
    radars = { "ZSU-23-4 Shilka" },
    type = "ZSU-23-4 Shilka"
  },
  ara_vdm = {
    DisplayName = "ARA Veinticinco de Mayo",
    frequencies = { { 50000000, 54000000 }, { 2000000000, 2200000000 } },
    radars = { "Osa 9A33 ln", "rezki search radar", "carrier search radar" },
    type = "ara_vdm"
  },
  atconveyor = {
    DisplayName = "SS Atlantic Conveyor",
    frequencies = {},
    radars = { "Osa 9A33 ln", "rezki search radar", "carrier search radar" },
    type = "atconveyor"
  },
  hms_invincible = {
    DisplayName = "HMS Invincible (R05)",
    frequencies = {},
    radars = { "Osa 9A33 ln", "rezki search radar", "carrier search radar" },
    type = "hms_invincible"
  },
  ["leander-gun-achilles"] = {
    DisplayName = "HMS Achilles (F12)",
    frequencies = {},
    radars = { "Patriot str" },
    type = "leander-gun-achilles"
  },
  ["leander-gun-andromeda"] = {
    DisplayName = "HMS Andromeda (F57)",
    frequencies = {},
    radars = { "Patriot str" },
    type = "leander-gun-andromeda"
  },
  ["leander-gun-ariadne"] = {
    DisplayName = "HMS Ariadne (F72)",
    frequencies = {},
    radars = { "Patriot str" },
    type = "leander-gun-ariadne"
  },
  ["leander-gun-condell"] = {
    DisplayName = "CNS Almirante Condell (PFG-06)",
    frequencies = {},
    radars = { "ticonderoga search radar", "Patriot str" },
    type = "leander-gun-condell"
  },
  ["leander-gun-lynch"] = {
    DisplayName = "CNS Almirante Lynch (PFG-07)",
    frequencies = {},
    radars = { "ticonderoga search radar", "Patriot str" },
    type = "leander-gun-lynch"
  },
  ["p-19 s-125 sr"] = {
    DisplayName = 'SAM SA-2/3/5 P19 "Flat Face" SR ',
    frequencies = { { 830000000, 875000000 } },
    radars = { "p-19 s-125 sr" },
    type = "p-19 s-125 sr"
  },
  rapier_fsa_blindfire_radar = {
    DisplayName = "SAM Rapier Blindfire TR",
    frequencies = { { 3000000000, 4000000000 } },
    radars = { "Tor 9A331" },
    type = "rapier_fsa_blindfire_radar"
  },
  rapier_fsa_launcher = {
    DisplayName = "SAM Rapier LN",
    frequencies = { { 1000000000, 4000000000 } },
    radars = { "Roland Radar" },
    type = "rapier_fsa_launcher"
  },
  santafe = {
    DisplayName = "ARA Santa Fe S-21",
    frequencies = { { 50000000, 54000000 }, { 2000000000, 2200000000 } },
    radars = { "PLAN Search Radar B", "052B SAM TR" },
    type = "santafe"
  },
  ["snr s-125 tr"] = {
    DisplayName = 'SAM SA-3 S-125 "Low Blow" TR',
    frequencies = { { 9000000000, 9400000000 } },
    radars = { "snr s-125 tr" },
    type = "snr s-125 tr"
  }
}
